// client/src/store/issueResolutionSlice.ts
...
> {
  const result = await consumeAgentStream<string>(
    ...
    {
      onPhase: () =>
        dispatch(issueResolutionSlice.actions.phaseChanged("running")),
      ...
      onAuthRequired: (authUrls) =>
        dispatch(issueResolutionSlice.actions.authRequired(authUrls)),

      onQuestionRequired: (question) =>
        dispatch(issueResolutionSlice.actions.questionRequired(question)),
    },
    body,
    { parseAs: "text" },
  );
...
/**
 * Phase A — the Deep Dive investigation.
...
>("issueResolution/start", async (issueUrl, { dispatch, rejectWithValue }) => {
  ...
  const result = await runTextStream(`${API_BASE}/stream`, dispatch, {
    issueUrl,
  });
...
/**
 * The human approval gate — only fires from
...
>(
  "issueResolution/approve",
  async (_, { dispatch, getState, rejectWithValue }) => {
    const issueUrl = getState().issueResolution.issueUrl;

    if (!issueUrl) {
      return rejectWithValue("No investigation to approve");
    }

    dispatch(issueResolutionSlice.actions.implementationApproved());

    const result = await runTextStream(
      `${API_BASE}/stream/continue`,
      dispatch,
      {
        issueUrl,
...
export const resumeIssueResolutionStream = createAsyncThunk<
  ...
  { state: RootState; rejectValue: string }
>(
  "issueResolution/resume",
  async (_, { dispatch, getState, rejectWithValue }) => {
    const issueUrl = getState().issueResolution.issueUrl;

    const result = await runTextStream(`${API_BASE}/stream/resume`, dispatch, {
      issueUrl,
    });
    ...
    if ("authUrls" in result || "question" in result) {
      return rejectWithValue("Still waiting on authorization");
    }
    ...
    await commitReport(
      issueUrl,
      phase === "implementing" ? "done" : "awaiting_approval",
      result.text,
    );
...
>(
  ...
  async (answer, { dispatch, getState, rejectWithValue }) => {
    ...
    if (!pending) {
      return rejectWithValue("No pending question to answer");
    }
...
export const fetchCachedIssueResolution = createAsyncThunk(
  ...
  async (issueUrl: string) => {
    const res = await fetch(
      `${API_BASE}?issueUrl=${encodeURIComponent(issueUrl)}`,
      {
        credentials: "include",
        headers: {
          Authorization: `Bearer ${localStorage.getItem("accessToken")}`,
...
function applyResult(
  ...
  text: string,
  nextPhase: "awaiting_approval" | "done",
) {
...
const issueResolutionSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      .addCase(startIssueResolution.pending, (state) => {
        state.status = "connecting";
      })

      .addCase(startIssueResolution.fulfilled, (state, action) => {
        if (action.payload && "text" in action.payload) {
          applyResult(state, action.payload.text, "awaiting_approval");
        }
      })
      ...
      .addCase(startIssueResolution.rejected, (state, action) => {
        state.status = "failed";
        state.error =
          action.payload ?? action.error.message ?? "Investigation failed.";
      })

      .addCase(approveIssueResolution.fulfilled, (state, action) => {
        if (action.payload && "text" in action.payload) {
          applyResult(state, action.payload.text, "done");
        }
      })
      ...
      .addCase(approveIssueResolution.rejected, (state, action) => {
        state.status = "failed";
        state.error =
          action.payload ?? action.error.message ?? "Implementation failed.";
      })
      ...
      .addCase(resumeIssueResolutionStream.fulfilled, (state, action) => {
        if (action.payload && "text" in action.payload) {
          applyResult(
            state,
            action.payload.text,
            state.phase === "implementing" ? "done" : "awaiting_approval",
          );
        }
      })
      ...
      .addCase(resumeIssueResolutionStream.rejected, (state, action) => {
        state.status = "failed";
        state.error =
          action.payload ?? action.error.message ?? "Failed to resume.";
      })
...
export const {
  ...
  streamReset: selectIssueForResolution,
} = issueResolutionSlice.actions;

export default issueResolutionSlice.reducer;

export const selectIssueResolution = (state: RootState) =>
