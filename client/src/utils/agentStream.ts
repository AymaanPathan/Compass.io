...
export interface AgentStreamOptions {
   ...
   *   JSON.parse it and surface data: T.
   *
   * - "text": the agent's final output is plain text/markdown.
 ...
 * output once turn_done arrives — either JSON-parsed or as raw text,
 * depending on `options.parseAs`.
 *
    ...
    switch (event.type) {
      ...
      case "turn_done":
        if (event.status === "error") {
          result = {
            kind: "error",
            message: "Agent turn ended in error.",
          };
        } else if (event.status === "cancelled") {
          // Don't attempt to parse rawBuffer here.
          // A cancelled turn may contain incomplete output.
